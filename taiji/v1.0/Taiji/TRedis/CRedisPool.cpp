...
CRedisClient *CRedisPool::_getConn(long millisecond)
{
...
void CRedisPool::_pushBackConn( CRedisClient *&pConn )
{
    if ( _status != REDIS_POOL_WORKING )
        return;
    Poco::Mutex::ScopedLock lock(_mutex);
...
void CRedisPool::_pushBackConn( int32_t connNum )
{
    if ( _status != REDIS_POOL_WORKING )
        return;
    Poco::Mutex::ScopedLock lock(_mutex);
...
{
    int32_t place = -1;
    CRedisClient* predis = _getConn( place,millisecond );
    if (NULL == predis)
    {
        throw ExceptHandle("lack handle");
    }
